...
gemiddelde van M-waarden: verwijderen omzetten van de laagste en hoogste waarden.
```{r normaliseren}
y <- calcNormFactors(y, method="TMM")
```

### Filteren op de low read counts
Van de genormaliseerde data worden de gene uit de data set verwijderd als ze onder
het gemiddelde eerste kwartiel vallen. Her gemiddelde is berekend over alle acht
de experimenten. Als de waarde er bovenligt dan is die TRUE als je er ondervalt 
dan is die FALSE. Alle genen die een FALSE hebben gekregen worden niet bewaard in
de data set.
<br>
Output: een samenvatting welke onderander de kwartielen gemiddelde, hoogste en 
...
namelijk Hiërarchies geclusterd en via de methode "K means". Er is vervolgens ook
nog met de methode "K means" een clustering gemaakt met alle genen. 
<br>
Output: de eerste grafiek geeft de Hiërarchies geclusteren weer, waarin gezien kan 
...
weer in een data frame gezet. Er is uiteindelijk ee data frame voor WCFS1 en NC8.
...
## KEGG mapper
Met de overgebleven gene zijn KEGG pathways bekeken aan de hand van de fold 
change waardes. Er is voor zowel WCFS1 en NC8 vier pathways bekeken, namelijk Glycolysis_Gluconeogenesis, citroenzuurcyclus, pentosefosfaat en oxidatieve 
fosforylering. 
<br>
