...
fDir <-  "C:/Users/luche/Documents/HAN/Leerjaar_2/Tutor/Blok 6/"
fNameWCFS1 <- "WCFS1_cnts.txt"
fNameNC8 <- "WCFS1_RNA-Seq.txt"

cntsWCFS1 <- read.delim(paste0(fDir,fNameWCFS1), comment.char="#")
cntsNC8 <- read.delim(paste0(fDir,fNameNC8), comment.char="#")

names(cntsNC8)[2] <- "NC8.glc.1"
names(cntsNC8)[3] <- "NC8.glc.2"
names(cntsNC8)[4] <- "NC8.rib.1"
names(cntsNC8)[5] <- "NC8.rib.2"

allecnts <- merge(cntsWCFS1,cntsNC8,by="ID")

row.names(allecnts) <- allecnts[,"ID"]
...
print(group)
print(allecnts[,4:9])
y <- DGEList(allecounts=allecnts[,2:9],group=group)
```
...
```{r beste resultaten}
print(topTags(fit))
res<-topTags(fit, n = length(fit$fitted.values))
```
...
write.xlsx(res, "foldChanges.xlsx", sheetName = "Highest Values", col.names = T,
