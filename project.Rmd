...
plot(test, fitWCFS1$table$logFC)

ble <- fitWCFS1$table$logFC
data <- data.frame(fitWCFS1)

ggplot(data, aes(x = test, y = logFC))  + geom_point() + scale_y_continuous(breaks = round(seq(min(data), max(data), by = 0.5),0))
...
plot(test, fitNC8$table$logFC, ygap.axis = 1)
...
#pathview(gene.data = logFC, species = "lpl", pathway = "lpl01100",  gene.idtype = "KEGG", out.suffix = "WCFS1.all")
...
#pathview(gene.data = logFC, species = "lpl", pathway = "lpl01100",  gene.idtype = "KEGG", out.suffix = "NC8.all")
```
