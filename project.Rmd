...
firstQuartile <- c()
for(i in 2:9) {
  firstQuartile[i-1] <- unname(quantile(cnts[,i], .25))
}
highEnough <- cnts[,2:9] > mean(firstQuartile)
...
## Genen Clusteren
```{r clusteren}
dis_matrix <- dist(resWCFS1$table$logFC, method = "euclidean")
hclust_ding <- hclust(dis_matrix, method = "average")
plot(hclust_ding)
